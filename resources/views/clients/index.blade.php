...
<form method="GET" action="{{ route('clients.index') }}" class="mb-6">
    ...
    <div class="radio-group flex items-center mb-3">
        ...
        <input type="radio" id="status_all" name="status" value="" {{ empty(request('status')) ? 'checked' : '' }}>
        <label for="status_all" class="mr-3">All</label>

        <input type="radio" id="status_lead" name="status" value="lead" {{ request('status') === 'lead' ? 'checked' : '' }}>
        <label for="status_lead" class="mr-3">Lead</label>

        <input type="radio" id="status_contacted" name="status" value="contacted" {{ request('status') === 'contacted' ? 'checked' : '' }}>
        <label for="status_contacted" class="mr-3">Contacted</label>

        <input type="radio" id="status_interested" name="status" value="interested" {{ request('status') === 'interested' ? 'checked' : '' }}>
        <label for="status_interested" class="mr-3">Interested</label>

        <input type="radio" id="status_negotiation" name="status" value="negotiation" {{ request('status') === 'negotiation' ? 'checked' : '' }}>
        <label for="status_negotiation" class="mr-3">Negotiation</label>

        <input type="radio" id="status_deal_made" name="status" value="deal_made" {{ request('status') === 'deal_made' ? 'checked' : '' }}>
    ...
    @foreach (is_array(request('search')) ? request('search') : (request('search') ? [request('search')] : []) as $search)
        @if ($search != '')
            <span class="tag bg-gray-400 text-white px-2 py-1 m-1 rounded">
                {{ $search }}
                <a href="{{ url('clients') }}?{{ http_build_query(array_merge(request()->except('search'), ['search' => array_diff(is_array(request('search')) ? request('search') : [request('search')], [$search])])) }}" class="ml-1 text-white">x</a>
            </span>
        @endif
    @endforeach
</div>
                    ...
                    <form id="importForm" action="{{ route('clients.import') }}" method="post" enctype="multipart/form-data" class="mb-6 hidden">
                        @csrf
                        ...
                        <div class="bg-white shadow-md rounded-lg overflow-hidden">
                            <div class="px-6 py-4">
                                ...
                                <div class="tags-container mt-2">
                                    ...
                                    <button id="moreTagsButton-{{ $client->id }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded" type="button" onclick="toggleTags({{ $client->id }}, event);">
                                        +{{ $client->tags->count() - 5 }}
                                    </button>
                            ...
                            <div class="px-6 py-2 border-t border-gray-200 flex items-center justify-between">
                                <div class="status-dropdown flex-grow mr-4">
                                    <form method="POST" action="{{ route('clients.updateStatus', $client) }}">
                                        @csrf
                                        <select name="status" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" onchange="this.form.submit()">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <form method="POST" action="{{ route('clients.destroy', $client) }}" onsubmit="return confirm('Are you sure you want to delete this item?');" class="ml-2">
                                        @csrf
                                        @method('DELETE')
    ...
    $('select[name="status"]').change(function() {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.log('Error updating status', textStatus, errorThrown);
            }
        });
    });
...
const searchInput = document.querySelector('input[name="search"]');
    ...
    radios.forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            let searchTags = Array.from(document.querySelectorAll('input[name="search[]"]')).map(input => input.value);
